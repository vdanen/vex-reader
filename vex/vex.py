...
class Vex(object):
    ...
    def __init__(self, vexfile):
        ...
        if is_json(vexfile):
            ...
            if response.status_code == 200:
                ...
                raise ConnectionError(
                    f"Cannot load {vexfile}. Response code: {response.status_code}"
                )
        ...
        if self.csaf["type"] != "csaf_vex":
            raise ValueError(
                f"Sorry, I can only handle csaf_vex 2.0 documents, this one is {self.csaf['type']} {self.csaf['csaf_version']}"
            )
        ...
        if "aggregate_severity" in self.raw["document"]:
            self.global_impact = self.raw["document"]["aggregate_severity"][
                "text"
            ].capitalize()
        ...
        if self.raw["document"]["tracking"]["current_release_date"].endswith("Z"):
            parsed_date = datetime.fromisoformat(
                self.raw["document"]["tracking"]["current_release_date"][:-1]
            )
            ud = parsed_date.replace(tzinfo=pytz.timezone("UTC"))
        else:
            ud = datetime.fromisoformat(
                self.raw["document"]["tracking"]["current_release_date"]
            )
        self.updated = ud.astimezone(pytz.timezone(TZ)).strftime(
            "%B %d, %Y at %I:%M:%S %p UTC"
        )

        if self.raw["document"]["tracking"]["initial_release_date"].endswith("Z"):
            parsed_date = datetime.fromisoformat(
                self.raw["document"]["tracking"]["initial_release_date"][:-1]
            )
            id = parsed_date.replace(tzinfo=pytz.timezone("UTC"))
        else:
            id = datetime.fromisoformat(
                self.raw["document"]["tracking"]["initial_release_date"]
            )
        ...
        if not self.acks:
            ...
            if (
                self.publisher == "Red Hat Product Security"
                and not self.acks_is_summary
            ):
                # throw the Red Hat-ism in here (maybe they could do like Cisco does as one statement?)
                self.acks = (
                    f"Red Hat would like to thank {self.acks} for reporting this issue."
                )
    ...
    def parse_vulns(self):
        ...
        for k in self.raw["vulnerabilities"]:
            ...
            if "release_date" in k:
                ...
                self.release_date = rd.astimezone(pytz.timezone(TZ)).strftime(
                    "%Y-%m-%d"
                )
            ...
            if "threats" in k:
                for x in k["threats"]:
                    ...
                    if "date" in x and not self.release_date:
                        ...
                        self.release_date = xd.astimezone(pytz.timezone(TZ)).strftime(
                            "%Y-%m-%d"
                        )
        ...
        if "ids" in k:
            for x in k["ids"]:
                if x["system_name"] == "Red Hat Bugzilla ID":
                    self.bz_id = x["text"]
                    self.bz_url = (
                        f"https://bugzilla.redhat.com/show_bug.cgi?id={self.bz_id}"
                    )
            ...
            for x in k["scores"]:
                ...
                if filtered_products and "cvss_v3" in x:
                    self.cvss_v3.append(
                        {
                            "scores": x["cvss_v3"],
                            "version": x["cvss_v3"]["version"],
                            "products": filtered_products,
                        }
                    )
                elif filtered_products and "cvss_v2" in x:
                    self.cvss_v2.append(
                        {"scores": x["cvss_v2"], "products": filtered_products}
                    )
        ...
        if self.cvss_v3:
            ...
            if len(self.cvss_v3) == 1:
                self.global_cvss = CVSSv3(
                    self.cvss_v3[0]["scores"], self.cvss_v3[0]["version"]
                )
