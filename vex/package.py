...
class VexPackages(object):
                    ...
                    if "branches" in b:
                        for c in b["branches"]:
                            ...
                            if "product" in c:
                                ...
                                if "product_identification_helper" in c["product"]:
                                    if (
                                        "cpe"
                                        in c["product"]["product_identification_helper"]
                                    ):
                                        cpe = c["product"][
                                ...
                                self.pmap.append(
                                    {id: {"name": name, "cpe": cpe, "purl": purl}}
                                )
                    elif "product" in b:
                        ...
                        if "product_identification_helper" in b["product"]:
                            if "cpe" in b["product"]["product_identification_helper"]:
                                cpe = b["product"]["product_identification_helper"][
                                    "cpe"
                                ]
                            if "purl" in b["product"]["product_identification_helper"]:
                                purl = b["product"]["product_identification_helper"][
                                    "purl"
                                ]
                        self.pmap.append({id: {"name": name, "cpe": cpe, "purl": purl}})
                # Handle product_version with direct product object
                elif "category" in b and (
                    b["category"] == "product_version"
                    or b["category"] == "product_version_range"
                ):
                    if "product" in b:
                        name = b["name"]
                        cpe = None
    ...
    def parse_packages(self):
        ...
        for k in self.raw["vulnerabilities"]:
            ...
            if (
                "remediations" in k
                and "product_status" in k
                and "fixed" in k["product_status"]
            ):
                ...
                if vendor_fix_ids and not any(
                    pid in fixed_set for pid in vendor_fix_ids
                ):
            ...
            if "product_status" in k:
                for x in k["product_status"]:
                    ...
                    if x == "known_affected":
                        for y in filter_components(
                            k["product_status"]["known_affected"]
                        ):
                            self.affected.append(Affected(y, self.pmap))
                    if x == "known_not_affected":
                        for y in filter_components(
                            k["product_status"]["known_not_affected"]
                        ):
                            ...
                            for na in self.not_affected:
                                if strip_arch(y) in na.raw:
                                    addMe = False
                            if addMe:
                                self.not_affected.append(
                                    NotAffected(strip_arch(y), self.pmap)
                                )
