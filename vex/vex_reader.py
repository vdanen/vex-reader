...
def main():
    ...
    parser.add_argument(
        "--vex", dest="vex", metavar="FILE", help="VEX file to process", required=True
    )
    parser.add_argument(
        "--show-components",
        ...
        action="store_true",
        default=False,
        help="Show components in output",
    )
    parser.add_argument(
        "--no-nvd",
        dest="no_nvd",
    ...
    if args.no_nvd:
        ...
        response = requests.get(
            f"https://services.nvd.nist.gov/rest/json/cves/2.0?cveId={vex.cve}",
            timeout=10,
        )
    ...
    if vex.global_impact:
        console.print(
            f"Impact    : [{SEVERITY_COLOR[vex.global_impact]}]{vex.global_impact}[/{SEVERITY_COLOR[vex.global_impact]}]"
        )
    if vex.global_cvss:
        console.print(
            f"CVSS Score: [cyan]{vex.global_cvss.baseScore}[/cyan]", highlight=False
        )
    print()
    ...
    if packages.mitigation:
        ...
        if len(packages.mitigation) > 1:
            console.print(
                "[bold red]**WARNING**: MORE THAN ONE MITIGATION DISCOVERED![/bold red]"
            )
    ...
    if vex.global_cvss:
        ...
        if cvssVersion == "3.0" or cvssVersion == "3.1":
            ...
            if (
                hasattr(vex.global_cvss, "attackVector")
                and vex.global_cvss.attackVector
            ):
                print(
                    f"  {f'CVSS {cvssVersion} Base Score':24} {vex.global_cvss.baseScore:<{width}} {nvd_cvss.baseScore if has_nvd_data else 'N/A'}"
                )
                print(
                    f"  {'Attack Vector':24} {vex.global_cvss.attackVector:{width}} {nvd_cvss.attackVector if has_nvd_data else 'N/A'}"
                )
                print(
                    f"  {'Attack Complexity':24} {vex.global_cvss.attackComplexity:{width}} {nvd_cvss.attackComplexity if has_nvd_data else 'N/A'}"
                )
                print(
                    f"  {'Privileges Required':24} {vex.global_cvss.privilegesRequired:{width}} {nvd_cvss.privilegesRequired if has_nvd_data else 'N/A'}"
                )
                print(
                    f"  {'User Interaction':24} {vex.global_cvss.userInteraction:{width}} {nvd_cvss.userInteraction if has_nvd_data else 'N/A'}"
                )
                print(
                    f"  {'Scope':24} {vex.global_cvss.scope:{width}} {nvd_cvss.scope if has_nvd_data else 'N/A'}"
                )
        elif cvssVersion == "2.0":
            if (
                hasattr(vex.global_cvss, "accessVector")
                and vex.global_cvss.accessVector
            ):
                # not all VEX will break down the metrics
                print(
                    f"  {'CVSS v2 Base Score':24} {vex.global_cvss.baseScore:<{width}} {nvd_cvss.baseScore if has_nvd_data else 'N/A'}"
                )
                print(
                    f"  {'Access Vector':24} {vex.global_cvss.accessVector:{width}} {nvd_cvss.accessVector if has_nvd_data else 'N/A'}"
                )
                print(
                    f"  {'Access Complexity':24} {vex.global_cvss.accessComplexity:{width}} {nvd_cvss.accessComplexity if has_nvd_data else 'N/A'}"
                )
                print(
                    f"  {'Authentication':24} {vex.global_cvss.authentication:{width}} {nvd_cvss.authentication if has_nvd_data else 'N/A'}"
                )
        if (
            hasattr(vex.global_cvss, "confidentialityImpact")
            and vex.global_cvss.confidentialityImpact
        ):
            print(
                f"  {'Confidentiality Impact':24} {vex.global_cvss.confidentialityImpact:{width}} {nvd_cvss.confidentialityImpact if has_nvd_data else 'N/A'}"
            )
            print(
                f"  {'Integrity Impact':24} {vex.global_cvss.integrityImpact:{width}} {nvd_cvss.integrityImpact if has_nvd_data else 'N/A'}"
            )
            print(
                f"  {'Availability Impact':24} {vex.global_cvss.availabilityImpact:{width}} {nvd_cvss.availabilityImpact if has_nvd_data else 'N/A'}"
            )
        print()
